...
**Key references:** Header comments and implementation of the official example plugins (including the flat files and subdirectories such as the Composition API, High Level, and WorldMatrix examples). All examples and categories below come from the official plugins (official plugins (folder name varies by project)). User-Made Plugins are referenced only at high level in the separate references/user-made-plugins.md (links + summaries only, no code or internals).
